    ...
    <div className="bg-background pb-safe-nav lg:pb-0">
      <section className="relative bg-night text-white pt-24 lg:pt-32 pb-12 overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-5 lg:px-8">
          <Link to="/" className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.25em] text-white/60 hover:text-flame transition">
            <ArrowLeft className="h-3.5 w-3.5" /> На главную
          </Link>
          <div className="mt-6 text-[11px] uppercase tracking-[0.3em] text-flame font-mono-pro">Документы</div>
          <h1 className="mt-3 font-display tracking-tight text-balance" style={{ fontSize: "clamp(2rem, 6vw, 3.75rem)", lineHeight: 0.98 }}>
            {title}
          </h1>
          {updated && <div className="mt-4 text-xs text-white/45 font-mono-pro">Редакция {updated}</div>}
        </div>
      </section>
        ...
        <div className="mx-auto max-w-4xl px-5 lg:px-8 grid lg:grid-cols-12 gap-10">
          <aside className="lg:col-span-4">
            <div className="lg:sticky lg:top-24 space-y-2">
              <div className="text-[10px] uppercase tracking-[0.3em] text-ink/45 font-mono-pro mb-3">Разделы</div>
              {links.map((l) => (
                <Link
                  key={l.to}
                  to={l.to}
                  className="block text-sm text-ink/70 hover:text-flame transition"
                  activeProps={{ className: "block text-sm text-flame font-semibold" }}
                >
                  {l.label}
                </Link>
              ))}
